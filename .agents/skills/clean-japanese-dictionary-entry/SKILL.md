...
- 当 `words+kana` 与 `rome+detail+examples` 分别组成两个不同但各自合理的词条身份时，属于结构性串条。不得通过多数表决或自行猜测目标来修复，必须 `submit_original_with_issue`；用户明确指定目标身份时除外。
...
- `金子/かねこ` 与 `kinsu/金钱、金币` 同时出现，或 `建/けん` 与 `tate/建てる例句` 同时出现时，两组字段各自指向不同词条，必须原样提交并记录冲突。
